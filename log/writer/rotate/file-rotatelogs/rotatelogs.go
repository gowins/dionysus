...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("invalid strftime pattern: %s ", err.Error())
	}
	...
	if maxAge > 0 && rotationCount > 0 {
		return nil, fmt.Errorf("options MaxAge and RotationCount cannot be both set")
	}
...
func (rl *RotateLogs) Write(p []byte) (n int, err error) {
	...
	if err != nil {
		rl.mutex.Unlock()
		return 0, fmt.Errorf("failed to acquite target io.Writer: %s", err.Error())
	}
	rl.mutex.Unlock()
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to open file %s: %s", rl.pattern, err)
	}

	if err := rl.rotate_nolock(filename); err != nil {
		err = fmt.Errorf("failed to rotate: %s", err.Error())
		if bailOnRotateFail {
	...
	// GC will close unused file descriptor.
...
func (rl *RotateLogs) rotate_nolock(filename string) error {
	...
	if rl.linkName != "" {
		...
		if err := os.Symlink(filename, tmpLinkName); err != nil {
			return fmt.Errorf("failed to create new symlink: %s", err.Error())
		}

		if err := os.Rename(tmpLinkName, rl.linkName); err != nil {
			return fmt.Errorf("failed to rename new symlink: %s", err.Error())
		}
	}

	if rl.maxAge <= 0 && rl.rotationCount <= 0 {
		return fmt.Errorf("panic: maxAge and rotationCount are both set")
	}
